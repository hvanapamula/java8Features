...
public class Java8Ex {
	...
	public static void main(String[] args) {
		...
		System.out.println("-------------------print All Items of respective shipments with first skip-------------------");
		order.getShipments().stream().map(sh->sh.getItems()).skip(1).findFirst().get().forEach(System.out::println);
		System.out.println("--------------------------------------------------------------------");
		...
		System.out.println(sj.add(item.getItemId()).add(item.getItemType()));
		
		order.getShipments().stream();
		
		System.out.println("####################################################################################");
		
//		order.getShipments().stream().map(sh->sh.getItems()).map(it->it.forEach(it->{
//			if(it.getItemId().isEmpty()) {
//				throw new Exception("sadndkjasn");
//			}
//		});)
		List<Item> itemsNew = new ArrayList<>();
	items.forEach(i->{
		i.setItemId("445664564");
		itemsNew.add(i);
	});
	...
	private static Item setPrice(Item item) {
		...
		item.setPrice(round(item.getPrice()*1.1, 3)); // Rounding the decimals to 3 places 
		return item;
